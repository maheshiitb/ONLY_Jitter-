...
class ECC_Transformer(nn.Module):
    def __init__(self, args, dropout=0):
        ...
        self.L = 3  
        self.neigh_weights = nn.Parameter(torch.zeros(2*self.L + 1))
        nn.init.constant_(self.neigh_weights, 1.0)
    ...
    def forward(self, magnitude, syndrome, L=3):
        ...
        weights = torch.softmax(self.neigh_weights, dim=0)
        for i in range(n):
            for k in range(-L, L+1):
                j = i + k
                if 0 <= j < n:
                    weight_index = k + L
                    neigh_emb[:, i] += weights[weight_index] * base_emb[:, j]
