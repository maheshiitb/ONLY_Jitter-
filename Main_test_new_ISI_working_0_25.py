...
from Model_target_25 import ECC_Transformer
import numpy as np
...
class ECC_Dataset(data.Dataset):
    def __init__( self, code, sigma, len, zero_cw=True, sinc_span=8, timing_offset=0.1 ):
        self.code = code
        self.sigma = sigma
        self.len = len
        ...
        self.sinc_span = sinc_span
        Delta = 0.25
        self.timing_offset = Delta * (2 * torch.rand(1) - 1)
        # self.timing_offset = 0.1
        t = torch.arange(-sinc_span, sinc_span + 1).float()
        self.registered_t = t  

    def __len__(self):
        return self.len

    def sinc_interp(self, x_sign):
        n = x_sign.shape[0]
        device = x_sign.device
        t = self.registered_t.to(device)
        y = torch.zeros(n, device=device)
        for i in range(n):
            idx = torch.arange(n, device=device)
            tau = (i + self.timing_offset) - idx
            sinc_kernel = torch.sinc(tau)
            y[i] = torch.sum(x_sign * sinc_kernel)
        return y
    ...
    def __getitem__(self, index):
        if self.zero_cw is None:
            ...
            m = self.zero_word
            x = self.zero_cw
        x_sign = bin_to_sign(x).float()
        y_clean = self.sinc_interp(x_sign)
...
def train(model, device, train_loader, optimizer, epoch, LR):
    ...
    for batch_idx, (m, x, z, y, magnitude, syndrome) in enumerate(
        ...
        if (batch_idx+1) % 200 == 0 or batch_idx == len(train_loader) - 1:
            logging.info(
...
def test(model, device, test_loader_list, EbNo_range_test, min_FER=100):
    ...
    with torch.no_grad():
        for ii, test_loader in enumerate(test_loader_list):
            ...
            logging.info(
                f"Test Eb/N0={EbNo_range_test[ii]}  BER={test_ber/cum_count:.2e}  FER={test_fer/cum_count:.2e}"
            )
        ###
        logging.info('\nTest Loss ' + ' '.join(
...
if __name__ == '__main__':
    ...
    # Code args
    parser.add_argument('--code_type', type=str, default='CCSDS', choices=['BCH', 'POLAR', 'LDPC', 'CCSDS', 'MACKAY'])
    parser.add_argument('--code_k', type=int, default=64)
    parser.add_argument('--code_n', type=int, default=128)
    ...
    # ================= GPU VISIBILITY (ADDED) =================
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpus if args.gpus != "-1" else "0"
    # =========================================================
    ...
    model_dir = os.path.join('Results_ECCT',
                             args.code_type + '__Code_n_' + str(
                                 ...
                                 "%d_%m_%Y_%H_%M_%S_Model_test_new_ISI"))
